/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
} from "../../../common";

export interface AddressAliasHelperInterface extends utils.Interface {
  functions: {
    "OFFSET()": FunctionFragment;
  };

  getFunction(nameOrSignatureOrTopic: "OFFSET"): FunctionFragment;

  encodeFunctionData(functionFragment: "OFFSET", values?: undefined): string;

  decodeFunctionResult(functionFragment: "OFFSET", data: BytesLike): Result;

  events: {};
}

export interface AddressAliasHelper extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: AddressAliasHelperInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    OFFSET(overrides?: CallOverrides): Promise<[BigNumber]>;
  };

  OFFSET(overrides?: CallOverrides): Promise<BigNumber>;

  callStatic: {
    OFFSET(overrides?: CallOverrides): Promise<BigNumber>;
  };

  filters: {};

  estimateGas: {
    OFFSET(overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    OFFSET(overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
